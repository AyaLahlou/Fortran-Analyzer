...
class SimpleFortranAnalyzer:
    ...
    def parse_fortran_file(self, file_path: Path) -> Dict:
        ...
        for match in re.finditer(
            module_pattern, clean_content, re.MULTILINE | re.IGNORECASE
        ):
        ...
        for match in re.finditer(
            use_pattern, clean_content, re.MULTILINE | re.IGNORECASE
        ):
        ...
        for match in re.finditer(
            subroutine_pattern, clean_content, re.MULTILINE | re.IGNORECASE
        ):
        ...
        for match in re.finditer(
            function_pattern, clean_content, re.MULTILINE | re.IGNORECASE
        ):
        ...
        for match in re.finditer(
            type_pattern, clean_content, re.MULTILINE | re.IGNORECASE
        ):
    ...
    def analyze_project(self) -> Dict:
        ...
        results = {
            ...
            "dependencies": {
                "internal": {
                    k: list(v & defined_modules) for k, v in self.dependencies.items()
                },
    ...
    def calculate_complexity_metrics(self) -> Dict:
        ...
        for module_info in self.modules.values():
            procedures = len(module_info.get("subroutines", [])) + len(
        ...
        return {
            "average_procedures_per_module": (
                total_procedures / len(self.modules) if self.modules else 0
            ),
    ...
    def get_translation_recommendations(self) -> Dict:
        ...
        for module in self.modules:
            internal_deps = [
                dep for dep in self.dependencies.get(module, []) if dep in self.modules
            ]
...
def main():
    ...
    print(
        f"  Start with leaf modules: {recommendations['start_with_leaf_modules'][:3]}"
    )
    print(
        f"  High-priority utilities: {recommendations['high_priority_utilities'][:3]}"
    )
    ...
    if args.output_dir:
        ...
        with open(output_path / "analysis_summary.txt", "w") as f:
            ...
            f.write(f"\nRECOMMENDATIONS:\n")
            f.write(
                f"  Start with: {', '.join(recommendations['start_with_leaf_modules'][:5])}\n"
            )
            f.write(
                f"  Utilities: {', '.join(recommendations['high_priority_utilities'][:5])}\n"
            )
