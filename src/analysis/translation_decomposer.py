...
class TranslationUnitDecomposer:
    ...
    def _decompose_module(self, module_info: ModuleInfo) -> List[TranslationUnit]:
        ...
        try:
            with open(module_info.file_path, 'r', encoding='utf-8', errors='ignore') as f:
                lines = f.readlines()
    ...
    def _create_module_unit(self, module_info: ModuleInfo, lines: List[str]) -> Optional[TranslationUnit]:
    ...
    def _decompose_interfaces(self, module_info: ModuleInfo, lines: List[str]) -> List[TranslationUnit]:
        ...
        for match in re.finditer(interface_pattern, content, re.MULTILINE | re.DOTALL | re.IGNORECASE):
            ...
            unit_id = self._generate_unit_id(module_info.name, f"interface_{interface_name}")
    ...
    def _decompose_types(self, module_info: ModuleInfo, lines: List[str]) -> List[TranslationUnit]:
        ...
        for entity in module_info.entities:
            if entity.entity_type == 'type' and entity.line_start > 0:
                unit_id = self._generate_unit_id(module_info.name, f"type_{entity.name}")
                ...
                unit = TranslationUnit(
                    ...
                    line_count=entity.line_end - entity.line_start + 1 if entity.line_end > 0 else 0,
                    has_types=True
                )
                units.append(unit)
    ...
    def _decompose_procedures(self, module_info: ModuleInfo, lines: List[str]) -> List[TranslationUnit]:
        """Decompose procedures (subroutines and functions) into translation units."""
        units = []
        
        for entity in module_info.entities:
            if entity.entity_type in ['subroutine', 'function'] and entity.line_start > 0:
                procedure_units = self._decompose_single_procedure(entity, module_info, lines)
                units.extend(procedure_units)
        
        return units
    
    def _decompose_single_procedure(self, entity: FortranEntity, module_info: ModuleInfo, 
                                   lines: List[str]) -> List[TranslationUnit]:
        ...
        if line_count <= self.config.max_translation_unit_lines:
            ...
            unit_id = self._generate_unit_id(module_info.name, f"{entity.entity_type}_{entity.name}")
            ...
            chunks = self._split_procedure_into_chunks(procedure_lines, entity, module_info)
            
            # Create root unit for procedure signature and declarations
            root_unit_id = self._generate_unit_id(module_info.name, f"{entity.entity_type}_{entity.name}_root")
    ...
    def _split_procedure_into_chunks(self, procedure_lines: List[str], 
                                   entity: FortranEntity, module_info: ModuleInfo) -> List[Tuple[int, int, str]]:
        ...
        for i in range(declaration_end, len(procedure_lines)):
            ...
            should_end_chunk = (
                current_size >= self.config.max_translation_unit_lines or
                (current_size >= self.config.min_chunk_lines and self._is_logical_boundary(line))
            )
    ...
    def _find_declaration_end(self, procedure_lines: List[str]) -> int:
        ...
        for i, line in enumerate(procedure_lines):
            ...
            executable_keywords = [
                'call', 'if', 'do', 'select', 'where', 'forall',
                'go to', 'goto', 'return', 'stop', 'cycle', 'exit'
            ]
            ...
            if '=' in line_clean and not any(decl in line_clean for decl in 
                                           ['integer', 'real', 'character', 'logical', 'type']):
                return i
    ...
    def _find_module_bounds(self, lines: List[str], module_name: str) -> Tuple[int, int]:
    ...
    def _analyze_unit_dependencies(self) -> None:
        ...
        for unit in self.translation_units:
            if unit.parent_id:
                parent = next((u for u in self.translation_units if u.id == unit.parent_id), None)
                if parent:
    ...
    def get_units_by_priority(self) -> List[TranslationUnit]:
        ...
        return sorted(self.translation_units, key=lambda u: (u.priority, u.complexity_score))
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Count by type
        for unit in self.translation_units:
            stats['units_by_type'][unit.unit_type] = stats['units_by_type'].get(unit.unit_type, 0) + 1
            stats['units_by_priority'][unit.priority] = stats['units_by_priority'].get(unit.priority, 0) + 1
            stats['units_by_effort'][unit.estimated_effort] = stats['units_by_effort'].get(unit.estimated_effort, 0) + 1
            stats['total_lines'] += unit.line_count
        
        stats['average_lines_per_unit'] = stats['total_lines'] / len(self.translation_units)
        
        # Find largest and most complex units
        stats['largest_unit'] = max(self.translation_units, key=lambda u: u.line_count).id
        stats['most_complex_unit'] = max(self.translation_units, key=lambda u: u.complexity_score).id
    ...
    def export_units(self, output_path: Path, format: str = 'json') -> None:
        ...
        logger.info(f"Exported {len(self.translation_units)} translation units to {output_path}")