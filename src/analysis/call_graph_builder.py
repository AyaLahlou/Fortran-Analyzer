...
class CallGraphBuilder:
                ...
                if dep_module in modules:
                    ...
                    if self.config.track_dependencies:
                        ...
                        self.module_graph.add_edge(
                            module_name,
                            dep_module,
                            dependency_type='external_use'
                        )
        
        logger.info(f"Module graph: {self.module_graph.number_of_nodes()} nodes, "
    ...
    def build_entity_call_graph(self, modules: Dict[str, ModuleInfo]) -> nx.DiGraph:
        ...
        logger.info(f"Entity graph: {self.entity_graph.number_of_nodes()} nodes, "
    ...
    def _extract_call_relationships(self, modules: Dict[str, ModuleInfo]) -> None:
        ...
        for module_name, module_info in modules.items():
            try:
                ...
                for entity in module_info.entities:
                    if entity.entity_type in ['subroutine', 'function']:
                        ...
                        if entity.line_start > 0 and entity.line_end > 0:
                            ...
                            entity_calls = re.findall(call_pattern, entity_content, re.IGNORECASE)
                            
                            for called_proc in entity_calls:
                                # Try to find the called procedure in our modules
                                for target_module_name, target_module in modules.items():
                                    if (called_proc in target_module.subroutines or 
                                        called_proc in target_module.functions):
                                        callee_id = f"{target_module_name}::{called_proc}"
                ...
                logger.warning(f"Failed to extract calls from {module_info.file_path}: {e}")
    ...
    def analyze_dependencies(self) -> Dict[str, Any]:
        ...
        orphaned = [
            n for n in self.module_graph.nodes()
            if (self.module_graph.in_degree(n) == 0 and 
                self.module_graph.out_degree(n) == 0 and
                not self.module_graph.nodes[n].get('external', False))
        ]
        analysis['orphaned_modules'] = orphaned
        
        # Find hub modules (high in-degree)
        in_degrees = dict(self.module_graph.in_degree())
        hub_threshold = max(2, len(self.module_graph.nodes()) * 0.1)  # At least 10% of modules
        hubs = [n for n, degree in in_degrees.items() 
                if degree >= hub_threshold and not self.module_graph.nodes[n].get('external', False)]
        analysis['hub_modules'] = sorted(hubs, key=lambda x: in_degrees[x], reverse=True)
        
        # Find leaf modules (no outgoing dependencies)
        leaves = [
            n for n in self.module_graph.nodes()
            if (self.module_graph.out_degree(n) == 0 and
                not self.module_graph.nodes[n].get('external', False))
        ]
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        if self.module_graph.nodes():
            ...
            metrics['module_graph'] = {
                ...
                'average_degree': sum(dict(self.module_graph.degree()).values()) / self.module_graph.number_of_nodes(),
            ...
            try:
                metrics['module_graph']['in_degree_centrality'] = nx.in_degree_centrality(self.module_graph)
                metrics['module_graph']['out_degree_centrality'] = nx.out_degree_centrality(self.module_graph)
                metrics['module_graph']['betweenness_centrality'] = nx.betweenness_centrality(self.module_graph)
            except:
        ...
        if nx.is_directed_acyclic_graph(internal_graph):
            ...
            while remaining:
                ...
                candidates = [
                    ...
                    if all(pred not in remaining for pred in internal_graph.predecessors(n))
                ]
                
                if not candidates:
                    # Break cycles by choosing node with lowest in-degree
                    candidates = [min(remaining, key=lambda x: internal_graph.in_degree(x))]
                
                # Sort candidates by some priority (e.g., line count, complexity)
                candidates.sort(key=lambda x: self.module_graph.nodes[x].get('line_count', 0))
    ...
    def export_graphs(self, output_dir: Path, formats: List[str] = ['graphml', 'gexf']) -> Dict[str, str]:
        ...
        for graph_name, graph in [('module_graph', self.module_graph), 
