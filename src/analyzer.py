...
class FortranAnalyzer:
    ...
    def analyze(self, save_results: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            module_graph = self.call_graph_builder.build_module_dependency_graph(modules)
            ...
            if self.visualizer and self.config.generate_graphs:
                ...
                visualization_files = self.visualizer.generate_all_visualizations(analysis_results)
    ...
    def _generate_recommendations(self, dependency_analysis: Dict, translation_stats: Dict,
                                modules: Dict) -> Dict[str, Any]:
    ...
    def _save_summary_report(self) -> None:
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"Analysis Date: {time.ctime(self.results['config']['analysis_timestamp'])}\n")
            f.write(f"Duration: {self.results['config']['analysis_duration']:.2f} seconds\n\n")
            ...
            f.write(f"  Module Dependencies: {dependencies.get('module_graph_summary', {}).get('edges', 0)}\n")
            f.write(f"  Circular Dependencies: {len(analysis.get('circular_dependencies', []))}\n")
            f.write(f"  External Dependencies: {len(analysis.get('external_dependencies', []))}\n")
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        return {
            'files': self.results.get('parsing', {}).get('statistics', {}).get('total_files', 0),
            'lines': self.results.get('parsing', {}).get('statistics', {}).get('total_lines', 0),
            'modules': len(self.results.get('parsing', {}).get('modules', {})),
            'translation_units': self.results.get('translation', {}).get('units', 0),
            'dependencies': self.results.get('dependencies', {}).get('module_graph_summary', {}).get('edges', 0),
            'circular_dependencies': len(self.results.get('dependencies', {}).get('analysis', {}).get('circular_dependencies', [])),
            'external_dependencies': len(self.results.get('dependencies', {}).get('analysis', {}).get('external_dependencies', []))
        }
