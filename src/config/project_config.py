...
class FortranProjectConfig:
    ...
    # File patterns
    fortran_extensions: List[str] = field(default_factory=lambda: ['.f90', '.F90', '.f', '.F', '.f95', '.F95'])
    include_patterns: List[str] = field(default_factory=lambda: ['**/*.f90', '**/*.F90'])
    ...
    visualization: Dict[str, Union[str, bool, int]] = field(default_factory=lambda: {
    ...
    def __post_init__(self):
        ...
        self.include_dirs = [str(Path(self.project_root) / d) for d in self.include_dirs]
        self.exclude_dirs = [str(Path(self.project_root) / d) for d in self.exclude_dirs]
    ...
    def validate(self) -> bool:
        ...
        if not self.source_dirs:
            ...
            if not existing_dirs:
                errors.append(f"None of the source directories exist: {self.source_dirs}")
        
        # Validate Fortran standard
        valid_standards = ['f77', 'f90', 'f95', 'f2003', 'f2008']
        if self.fortran_standard not in valid_standards:
            errors.append(f"Invalid Fortran standard: {self.fortran_standard}. Must be one of: {valid_standards}")
...
class ConfigurationManager:
    ...
    TEMPLATES = {
        'ctsm': {
            ...
            'system_modules': ['shr_kind_mod', 'clm_varpar', 'clm_varctl', 'clm_varcon'],
    ...
    @classmethod
    def create_config_from_template(cls, template_name: str, project_root: str, 
    ...
    def auto_detect_project_type(cls, project_root: str) -> str:
        ...
        if (root_path / 'src' / 'biogeophys').exists() or \
           any(root_path.glob('**/*clm*')) or \
           any(root_path.glob('**/*ctsm*')):
            return 'ctsm'
        
        # Check for climate model indicators
        if (root_path / 'src' / 'physics').exists() and \
           (root_path / 'src' / 'dynamics').exists():
