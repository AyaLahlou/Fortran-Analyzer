...
class FortranVisualizer:
    ...
    def visualize_module_dependencies(self, call_graph_builder: CallGraphBuilder, 
        ...
        internal_nodes = [n for n in module_graph.nodes() 
        ...
        # Draw nodes
        node_sizes = [internal_graph.nodes[n].get('line_count', 100) / 10 + 100 
                     for n in internal_graph.nodes()]
        node_colors = [internal_graph.nodes[n].get('subroutines', 0) + 
        ...
        # Customize plot
        ax.set_title(f'{self.config.project_name} - Module Dependencies', 
    ...
    def visualize_translation_units(self, translation_units: List[TranslationUnit],
                                  save_path: Optional[Path] = None) -> Path:
        ...
        fig.suptitle(f'{self.config.project_name} - Translation Units Analysis', 
                    fontsize=16, fontweight='bold')
        ...
        if max(complexity_scores) > 0:
            ax3.hist(complexity_scores, bins=15, alpha=0.7, color='lightcoral', edgecolor='black')
        ...
        if priorities:
            ...
            bars = ax4.bar(priority_labels, priority_values, color='lightgreen', alpha=0.8, edgecolor='black')
            ...
            ax4.text(0.5, 0.5, 'No priority data', ha='center', va='center', transform=ax4.transAxes)
    ...
    def create_project_overview(self, statistics: Dict[str, Any],
                              save_path: Optional[Path] = None) -> Path:
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(f'{self.config.project_name} - Project Overview', 
                    fontsize=16, fontweight='bold')
        ...
        if any(basic_stats.values()):
            bars = ax1.bar(basic_stats.keys(), basic_stats.values(), 
            ...
            ax1.text(0.5, 0.5, 'No statistics available', ha='center', va='center', 
        ...
        if 'file_sizes' in statistics:
            ...
            ax2.hist(file_sizes, bins=20, alpha=0.7, color='lightblue', edgecolor='black')
            ax2.set_title('File Size Distribution')
            ax2.set_xlabel('Lines of Code')
            ax2.set_ylabel('Number of Files')
            ax2.grid(True, alpha=0.3)
        else:
            ax2.text(0.5, 0.5, 'File size data not available', ha='center', va='center', 
                    transform=ax2.transAxes)
            ax2.set_title('File Size Distribution')
        
        # 3. Module complexity (placeholder)
        ax3.text(0.5, 0.5, 'Module Complexity\n(Feature not implemented)', 
        ...
        if 'external_dependencies' in statistics:
            ...
            if ext_deps:
                ...
                ax4.text(0.5, 0.5, 'No external dependencies', ha='center', va='center', 
                        transform=ax4.transAxes)
                ax4.set_title('External Dependencies')
        else:
            ax4.text(0.5, 0.5, 'Dependency data not available', ha='center', va='center', 
                    transform=ax4.transAxes)
            ax4.set_title('External Dependencies')
    ...
    def create_interactive_dependency_graph(self, call_graph_builder: CallGraphBuilder,
        ...
        node_trace = go.Scatter(
            ...
            customdata=[[internal_graph.nodes[n].get('line_count', 0),
                        internal_graph.nodes[n].get('subroutines', 0),
                        internal_graph.nodes[n].get('functions', 0)] 
                       for n in internal_graph.nodes()],
            marker=dict(
                size=[max(10, internal_graph.nodes[n].get('line_count', 100) / 50) 
                     for n in internal_graph.nodes()],
                color=[internal_graph.nodes[n].get('subroutines', 0) + 
                      internal_graph.nodes[n].get('functions', 0) 
                      for n in internal_graph.nodes()],
                colorscale='Viridis',
                showscale=True,
    ...
    def create_translation_priority_chart(self, translation_units: List[TranslationUnit],
                                        save_path: Optional[Path] = None) -> Path:
        ...
        sorted_units = sorted(translation_units, key=lambda u: (u.priority, u.complexity_score))
        
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))
        fig.suptitle(f'{self.config.project_name} - Translation Priority Analysis', 
                    fontsize=16, fontweight='bold')
        ...
        scatter = ax1.scatter(priorities, complexities, s=line_counts, alpha=0.6, 
                            c=line_counts, cmap='viridis')
        ...
        unit_names = [f"{u.module_name}::{u.entity_name}"[:30] + "..." 
                     if len(f"{u.module_name}::{u.entity_name}") > 30 
                     else f"{u.module_name}::{u.entity_name}" 
                     for u in sorted_units[:20]]  # Show top 20
        ...
        legend_elements = [Patch(facecolor='lightgreen', label='Low Effort'),
    ...
    def generate_all_visualizations(self, analysis_results: Dict[str, Any]) -> Dict[str, Path]:
        ...
        try:
            ...
            if 'call_graph_builder' in analysis_results:
                path = self.visualize_module_dependencies(analysis_results['call_graph_builder'])
                if path:
                    generated_files['module_dependencies'] = path
                
                # Interactive version
                path = self.create_interactive_dependency_graph(analysis_results['call_graph_builder'])
                if path:
                    generated_files['interactive_dependencies'] = path
            
            # Translation units
            if 'translation_units' in analysis_results:
                path = self.visualize_translation_units(analysis_results['translation_units'])
                if path:
                    generated_files['translation_units'] = path
                
                path = self.create_translation_priority_chart(analysis_results['translation_units'])
                if path:
