...
def create_config_command(args) -> None:
    ...
    # Save configuration
    config_file = Path(args.output) if args.output else Path(args.project_root) / "fortran_analyzer_config.yaml"
...
Examples:
  ...
  fortran-analyzer info /path/to/project --detect --list-files
    ...
    config_parser.add_argument('--template', '-t', default='auto',
                              help='Configuration template (use "list" to see all)')
    config_parser.add_argument('--output', '-o', 
                              help='Output configuration file path')
    config_parser.add_argument('--project-name', 
    ...
    analyze_parser.add_argument('project_root', nargs='?',
                               help='Path to Fortran project root')
    analyze_parser.add_argument('--config', '-c',
                               help='Configuration file path')
    analyze_parser.add_argument('--template', '-t', default='auto',
                               help='Configuration template')
    analyze_parser.add_argument('--output-dir', '-o',
                               help='Output directory')
    analyze_parser.add_argument('--project-name',
                               help='Project name')
    analyze_parser.add_argument('--max-unit-lines', type=int,
                               help='Maximum lines per translation unit')
    analyze_parser.add_argument('--no-graphs', dest='graphs', action='store_false',
    ...
    info_parser.add_argument('--detect', action='store_true',
                            help='Auto-detect project type')
    info_parser.add_argument('--list-files', action='store_true',
                            help='List Fortran files in project')
    ...
    # Validate arguments
    if args.command in ['analyze', 'info'] and not args.project_root and not getattr(args, 'config', None):
