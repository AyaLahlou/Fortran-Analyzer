...
try:
    ...
    from fparser.two.Fortran2003 import (
        Module, Use_Stmt, Subroutine_Subprogram, Function_Subprogram,
        Derived_Type_Def, Subroutine_Stmt, Function_Stmt, Call_Stmt,
        Declaration_Construct, Variable_Declaration, Implicit_Stmt,
        Interface_Block, Abstract_Interface_Block
    )
...
class FortranParser:
    ...
    def find_fortran_files(self) -> List[Path]:
        ...
        fortran_files = [f for f in fortran_files 
                        if f.suffix in self.config.fortran_extensions]
    ...
    def _extract_module_info_regex(self, content: str, lines: List[str], 
                                  file_path: Path) -> ModuleInfo:
        ...
        # Extract module name
        module_match = re.search(r'^\s*module\s+(\w+)', content, re.MULTILINE | re.IGNORECASE)
        if module_match:
        ...
        for match in re.finditer(sub_pattern, content, re.MULTILINE | re.IGNORECASE):
            ...
            # Find line numbers
            line_start, line_end = self._find_entity_bounds(lines, subroutine_name, 'subroutine')
        ...
        for match in re.finditer(func_pattern, content, re.MULTILINE | re.IGNORECASE):
            ...
            line_start, line_end = self._find_entity_bounds(lines, function_name, 'function')
        ...
        for match in re.finditer(type_pattern, content, re.MULTILINE | re.IGNORECASE):
            ...
            if type_name.lower() not in ['public', 'private']:  # Skip access specifiers
                ...
                line_start, line_end = self._find_entity_bounds(lines, type_name, 'type')
    ...
    def _find_entity_bounds(self, lines: List[str], entity_name: str, 
    ...
    def _extract_use_stmt(self, use_node) -> Optional[Dict[str, Union[str, List[str]]]]:
        ...
        try:
            ...
            if len(use_node.children) > 2 and use_node.children[2]:
                # Has ONLY clause
                only_list = [str(item).strip() for item in use_node.children[2].children[1].children]
    ...
    def _extract_subroutine_info_fparser(self, sub_node, file_path: Path) -> Optional[FortranEntity]:
    ...
    def _extract_function_info_fparser(self, func_node, file_path: Path) -> Optional[FortranEntity]:
    ...
    def _extract_type_info_fparser(self, type_node, file_path: Path) -> Optional[FortranEntity]:
    ...
    def parse_project(self) -> Dict:
        ...
        logger.info(f"Analysis complete. Found {len(modules)} modules, {len(all_entities)} entities")
        return self.results
    ...
    def _build_dependency_graph(self, modules: Dict[str, ModuleInfo]) -> Dict:
        ...
        for module_name, module_info in modules.items():
            ...
            for use in module_info.uses:
                ...
                if self.config.system_modules and dep_module in self.config.system_modules:
                    continue
    ...
    def _calculate_statistics(self, modules: Dict[str, ModuleInfo], 
                            entities: List[FortranEntity]) -> Dict:
        ...
        return {
            ...
            'average_subroutines_per_module': total_subroutines / len(modules) if modules else 0
        }
